...
- (void)initQRLine {
    qrLine  = [[UIImageView alloc] initWithFrame:CGRectMake(self.bounds.size.width / 2 - self.transparentArea.width / 2, self.bounds.size.height / 2 - self.transparentArea.height / 2 - 100, self.transparentArea.width, 2)];
    ...
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(self.bounds.size.width / 2 - self.transparentArea.width / 2, self.bounds.size.height / 2 + 10, 200, 21)];
    label.text = @"将条形码放入框内，可自动扫描";
...
- (void)show {
    ...
    [UIView animateWithDuration:kQrLineanimateDuration animations:^{
        ...
        CGFloat maxBorder = self.frame.size.height / 2 + self.transparentArea.height / 2 - 4 - 100;
        if (qrLineY > maxBorder) {
            qrLineY = self.frame.size.height / 2 - self.transparentArea.height /2 - 100;
        }
        qrLineY++;
    }];
}
...
- (void)drawRect:(CGRect)rect {
    ...
    CGRect clearDrawRect = CGRectMake(screenDrawRect.size.width / 2 - self.transparentArea.width / 2,
                                      screenDrawRect.size.height / 2 - self.transparentArea.height / 2 - 100,
